...
function isIsoscelesTriangle(a, b, c) {
  const TRIANGLE = a + b <= c || b + c <= a || c + a <= b;
...
function isContainNumber(num, digit) {
  const STRIND_NUM = `${num}`;
  const STRING_DIGIT = `${digit}`;
  let result = false;

  for (let i = 0; i < STRIND_NUM.length; i += 1) {
    if (STRIND_NUM[i] === STRING_DIGIT) {
      result = true;
    }
  }
...
function getSpiralMatrix(size) {
  ...
  while (column <= endColumn && row <= endRow) {
    ...
    for (let i = endColumn; i >= column; i -= 1) {
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    ...
    for (let j = matrix[i].length - 1; j >= 0; j -= 1) {
      PART[matrix[i].length - j - 1] = matrix[j][i];
    }
    COPY[i] = PART;
  }
...
function sortByAsc(arr) {
  ...
  const COPY = [...sortByAsc(LEFT), PIVOT, ...sortByAsc(RIGHT)];
...
function getNearestBigger(number) {
  const numberArr = Array.from(`${number}`, (x) => +x);

  for (let i = numberArr.length - 1; i >= 0; i -= 1) {
    if (numberArr[i] < numberArr[i + 1]) {
      const left = [...numberArr].splice(0, i);
      const right = [...numberArr].splice(-numberArr.length + i).sort();

      left.push(right.find((el) => el > numberArr[i]));
      right.splice(
        right.findIndex((el) => el > numberArr[i]),
        1
      );
